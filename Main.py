...
def confirm_edited_record(tree, entries):
    ...
    if selected_record_id and name and item_type and quantity.isdigit():  # Validate inputs
        conn = sqlite3.connect("inventory.db")
        cursor = conn.cursor()
        ...
        selected_record_id = None  # Reset the global variable
    else:
        print("Invalid input or no record selected. Please check the fields.")
